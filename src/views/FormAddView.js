...
import {wget,wpost,wput} from './../actions/wawanajax';
...
class FormAddView extends React.Component{
	...
	handleSaveKontak(callback,scope){
		this.props.dispatch(addKontak(!scope.props.state.viewadd,callback));
		this.props.getData();
	}
	handleBatal(){
		...
		if (this.props.state.mode=='edit'){
			...
			renderEdit=(<div className="col-lg-12 boxadd">
							...
							<div className="col-xs-3 syle={{paddingTop:'20px'}}">
								<label>Picture</label>
								<input className="text-right"  type="file" onChange={(file)=>this.handleUpload(file)} ref={"picture"} />
								<input className="text-right"  defaultValue={this.props.state.dataEdit.version} type="text" hidden ref={"version"} />
							</div>
						</div>
			)
		}else {
			...
			renderEdit=(<div className="col-lg-12 boxadd">
							...
							<div className="col-xs-3 syle={{paddingTop:'20px'}}">
								<label>Picture</label>
								<input className="text-right"  type="file" ref={"picture"} onChange={(file)=>this.handleUpload(file)} />
